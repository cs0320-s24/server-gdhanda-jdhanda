package edu.brown.cs.student.census.caching;
...
public class CensusCachingTests {
  ...
  public void setup() {
    ...
    MockCensusSource mockedSource =
        new MockCensusSource(
            ...
            "Colorado",
            "Denver County");
    CachingCensusSource source = new CachingCensusSource(mockedSource);
   ...
   */
  @Test
  public void testCensusRequestTimeEquality() throws IOException, InterruptedException {
    ...
    Thread.sleep(1000); // Wait for time to pass

    // Set up the request, make the request
    HttpURLConnection loadConnection2 = tryRequest("broadband?state=Colorado&county=Denver+County");
    // Get an OK response (the *connection* worked, the *API* provides an error response)
    ...
    // Times are equivalent because cached.
    assertEquals(responseBody2.get("data"), responseBody.get("data"));
